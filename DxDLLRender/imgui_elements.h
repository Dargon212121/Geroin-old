...
bool Tab(const char* icon, const char* label, const char* desc, const ImVec2& size_arg, const bool selected)
{
	...
	if (selected)
		window->DrawList->AddRectFilled({ bb.Min.x,bb.Min.y }, { bb.Max.x,bb.Max.y }, ImColor(48, 23, 79));

	if (selected)
		window->DrawList->AddRectFilled({ bb.Max.x,bb.Max.y }, { bb.Max.x - 3,bb.Min.y }, ImColor(Colors::TextColor2));

	//	ImGui::PushFont(big);
	window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(label).y / 2), ImColor(Colors::Fonts), label);
	//	ImGui::PopFont();
...
bool SubTab(const char* label, const ImVec2& size_arg, const bool selected)
{
	...
	if (selected)
		window->DrawList->AddRectFilled({ bb.Max.x,bb.Max.y }, { bb.Max.x - 1,bb.Min.y }, ImColor(Colors::TextColor2));
