...
public:
	...
	static void RenderTracers() {
		for (int i = 0; i < bulletTracers.size(); i++) {
			...
			if (w2s(tracer.start, s_pos_start) && w2s(tracer.end, s_pos_end)) {
	...
	static void RenderTraceResults() {
		for (int i = 0; i < traceResults.size(); i++) {
			...
			Vector2 s_pos_end;
			if (w2s(tracer.hitPosition, s_pos_end)) {
				//draw_line(s_pos_start, s_pos_end);
				Render::Object.Circle(s_pos_end, D2D1::ColorF::Blue, 5.f);
			}
		}
	}
	static void RenderExplosions() {
		for (int i = 0; i < LogSystem::loggedExplosions.size(); i++) {
			...
			Vector2 explPos;
			if (w2s(explosion.position, explPos)) {
				Render::Object.Text(
					explPos,
					D2D1::ColorF::Red,
					true,
					...
					StringConverter::ToUnicode(StringFormat::format(xorstr("%s [%.2fm] [%d]"),
						...
						Math::Distance_3D(explosion.position, LocalPlayer.BasePlayer->GetBoneByID(head)),
...
void LogSystem::draw_text(Vector2 pos, std::wstring str) {
	Render::Object.Text(pos, D2D1::ColorF::White, false, true, str.c_str());
}
void LogSystem::draw_line(Vector2 pos, Vector2 pos2) {
	Render::Object.Line(pos, pos2, D2D1::ColorF::Red, 1.f, true);
}
